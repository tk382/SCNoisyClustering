...
#                 kernel_type = "pearson",
#                 verbose=FALSE)
load('analysis/slsl_dropseq.Rdata')
...
par(mfrow = c(2,3))
for (i in 1:6){
  plot(log.cpm[sorted_p$ix[i], sorted_type], col = rainbow(length(tab))[as.factor(out$result[sorted_type])],
...
ind = match(c('Timp3', 'Abca8a', 'Pde6g', 'Sparc', 'Rcvrn', 'Gngt2'), genenames)
df = data.frame(tsne1 = tsne$Y[,1], tsne2 = tsne$Y[,2],
                Timp3 = log.cpm[ind[1], ],
                Abca8a = log.cpm[ind[2], ],
                Pde6g = log.cpm[ind[3], ],
                Sparc = log.cpm[ind[4], ],
                Rcvrn = log.cpm[ind[5], ],
                Gngt2 = log.cpm[ind[6], ])
g1 = ggplot(df, aes(x=tsne1, y=tsne2, col = Timp3)) + geom_point(size=0.1) +
  scale_colour_gradient(low="pink", high="black") + guides(color = FALSE) + ggtitle('Timp3')
g2 = ggplot(df, aes(x=tsne1, y=tsne2, col = Abca8a)) + geom_point(size=0.1) +
  scale_colour_gradient(low="pink", high="black") + guides(color = FALSE) + ggtitle('Abca8a')
g3 = ggplot(df, aes(x=tsne1, y=tsne2, col = Pde6g)) + geom_point(size=0.1) +
  scale_colour_gradient(low="pink", high="black") + guides(color = FALSE) + ggtitle('Pde6g')
g4 = ggplot(df, aes(x=tsne1, y=tsne2, col = Sparc)) + geom_point(size=0.1) +
  scale_colour_gradient(low="pink", high="black") + guides(color = FALSE) + ggtitle('Sparc')
g5 = ggplot(df, aes(x=tsne1, y=tsne2, col = Rcvrn)) + geom_point(size=0.1) + scale_colour_gradient(low="pink", high="black") + guides(color = FALSE) + ggtitle('Rcvrn')
g6 = ggplot(df, aes(x=tsne1, y=tsne2, col = Gngt2)) + geom_point(size=0.1) + scale_colour_gradient(low="pink", high="black") + guides(color = FALSE) + ggtitle('Gngt2')
