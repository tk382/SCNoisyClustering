...
summ = data.frame(data       = rep(datanames, each=6),
                  method     = rep(c('pcaReduce','SIMLR',
                                     'sparseSL+fl','SC3','kmeans','sparseSL'), 8),
                  ARI      = rep(0, 48))
...
load('results_references/SC3result_clustering.Rdata')
for (i in 1:8){
  print(i)
  tmpX        = X[[i]]
  ...
  gc()
  # nClust      = numClust[[i]]
  # P           = mystery_kernel(t(tmpX), allk_input = c(20,30), sigma=c(2,1))
  # ssl[[i]]    = sparse_scaledlasso_c(P, 5, verbose=TRUE)
  # S           = ssl[[i]]$S
  # S2tmp       = gflars(ssl[[i]]$S, rep(1,ncol(tmpX)), nClust)
  # S2          = S2tmp$output[[nClust]]$theta
  # SS[[i]]     = (S2 + t(S2))/2
  # res[[i]]    = tsne_spectral(SS[[i]], nClust) #can fix numEigen argument
  # adj.rand.index(ind$x, res[[i]])
  # ours[[i]]   = nuclear_scaledlasso_c(P, 1, verbose=TRUE)
  # clust[[i]]  = tsne_spectral(ours[[i]]$S, nClust)
  # print('starting k..')
  # k[[i]]     = kmeans(t(tmpX), numClust[[i]], nstart=5)$cluster
  # s[[i]]           = SIMLR(tmpX, nClust)
  # print('pcareduce starting..')
  # pcareduce[[i]]   = PCAreduce(t(tmpX), 10, nClust, 'M')[[1]][,1]
  summ[summ$data==datanames[i] & summ$method=='pcaReduce', 'ARI'] =
    adj.rand.index(truth[[i]], clust[[i]])
  ...
  summ[summ$data==datanames[i] & summ$method=='SC3', 'ARI'] =
    adj.rand.index(truth[[i]], sc3_result[[i]])
  gc()
}
