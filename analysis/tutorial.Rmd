...
The two most important arguments are the data matrix "X" and reference panel "ref". We assume X is count matrix, while reference panel is already log transformed and normalized. Another parameter the user can choose is "knn". Empirical results showed that, after taking the projection of the data matrix onto the reference panel, removing low correlations improve the signal strength. When "knn" is set to TRUE, for each cell, SLSL_ref will only keep a few strongest correlations and push everything else to 0. The number of neighbors is determined by "knn_keep" argument. The number of clusters does not have to be identified. 
...
```{r pbmc_analyze}
heat(pbmc_ref$projection[,sort(pbmc_ref$result, index.return=T)$ix])

plot(tsne$Y, col = pbmc_ref$result)
```

When knn_keep is set to 1, the projection matrix will look like below. Only the celltype with the maximum correlation is left with signal while all others are pushed to 0. 
...
```
```{r}
heat(chu_ref$projection[,sort(chu_ref$result, index.return=T)$ix])
```


```{r ref_analyze, fig.width=3.5, fig.height=3.5}
heat(chu_ref$projection)
```
