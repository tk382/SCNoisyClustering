...
```{r similarity_matrix, fig.width=4, fig.height=4}
heat(out$S)
```
...
```{r dimred, fig.width=4, fig.height=4}
tsne = out$tsne
...
To demonstrate the use of the reference panel, we use the cell types of human embryonic cells published in [@chu2016single]. 
...
X = Chu_celltype$X
out_cell_ref = SLSL(X        = X, 
                    ref      = ref,
                    numClust = 7)
```


```{r ref_analyze, fig.width=4, fig.height=4}
...
```{r ref_analyze2, fig.width=4, fig.height=4}
...
```{r ref_analyze3, fig.width=4, fig.height=4}
...
```{r tissueref}
ref = reference_panel$tissue
out_tissue_ref = SLSL(X, 
                      ref = ref, 
                      numClust=7)
```
...
```{r tissueref_analyze, fig.width=4, fig.height=4}
...
## Parameter Set-Up
```{r zeisel_coverage, fig.width=4, fig.height=4}
