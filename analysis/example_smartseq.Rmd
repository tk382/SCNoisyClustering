...
plot(df$tsne1, df$tsne2, col = rainbow(6)[df$truelabel],
     xlab = 'tsne1', ylab = 'tsne2', main = 'True Label',
     cex = 0.5)
rm(pc)
```
...
out = SLSL(log.cpm, log=FALSE,
                ...
                verbose=FALSE)
plot(df$tsne1, df$tsne2, col = rainbow(5)[as.numeric(as.factor(out$result))],
     xlab = 'tsne1', ylab = 'tsne2', main = 'SLSL result',
     cex = 0.5)
...
adj.rand.index(as.numeric(as.factor(truelabel)),
               col_data_selected$sc3_6_clusters)
```
...
par(mfrow = c(2,3))
for (i in 1:6){
  plot(log.cpm[sorted_p$ix[i], sorted_type], col = rainbow(5)[ as.factor(out$result[sorted_type])],
