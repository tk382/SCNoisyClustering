...
ARI         = matrix(0,8,4)
measuretime = matrix(0,8,4)

kernel_list   = c('spearman','pearson','euclidean','SLSL')
data_list     = c('Kolod','Pollen','Usoskin','Buettner',
                  'Yan','Treutlein','Chu1','Chu2')

colnames(ARI) = kernel_list
rownames(ARI) = data_list
colnames(measuretime) = c('SLSL', 'SC3','pcaReduce', 'SIMLR')
rownames(measuretime) = data_list
```
...
stargazer(as.matrix(cbind(cellcount, measuretime[ind, ])))
