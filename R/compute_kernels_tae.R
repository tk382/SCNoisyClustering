...
multiple_kernel_tae_wrong = function(x, allk_input, sigma_input){
    ...
    allk = allk_input
    #D_Kernels = list()
    D_Kernels = array(0,dim=c(m,n,length(allk)*length(sigma)))
    for (l in 1:length(allk)){
      if(allk[l] < nrow(x)-1){
        TT = apply(Diff_sort[, 2:(allk[l]+1)], 1, mean) #mu
        TT = matrix(TT, nrow=length(TT), ncol=1) #mu in a column matrix
        Sig = apply(array(0,c(nrow(TT),ncol(TT))),1, function(x) {x=TT[,1]}) #repeat TT n times
        Sig = (Sig + t(Sig))/2 # Sig_{ij} = (mu_i + mu_j) / 2
        ...
        for (j in 1:length(sigma)){
          KK = KK+1
          Sigma2 = Sig * sigma[j]
          W = exp(-Diff/(Sigma2^2))/(Sigma2 * sqrt(2*pi))
...
multiple_kernel_tae = function(x, allk_input, sigma_input){
  N = dim(x)[1] #number of cells
  KK = 0
  sigma = sigma_input
  ...
  for (l in 1:length(allk)){
    if(allk[l] < nrow(x)-1){
      ...
      for (j in 1:length(sigma)){
        KK = KK+1
        Sigma2 = Sig * sigma[j]
        W = exp(-Diff^2/(Sigma2^2))
        #W2 = exp(-Diff^2/(Sigma2^2))/(Sigma2 * sqrt(2*pi))
        D_Kernels[,,KK] = (W + t(W))/2
      }
    }
  }
...
#compute and return the multiple kernel (reversed version)
multiple_kernel = function( x, cores.ratio = 1 ) {
  # set the parameters
  kernel.type = list()
  ...
  N = dim(x)[1] #number of cells
  KK = 0
  sigma = seq(2,1,-0.25)  #sigma for gaussian kernel i guess
  ...
  D_Kernels = unlist(parLapply(cl,1:length(allk),fun=function(l,x_fun=x,Diff_sort_fun=Diff_sort,allk_fun=allk,
    ...
    if(allk_fun[l]<(nrow(x_fun)-1)) {
      ...
      for (j in 1:length(sigma_fun)) {
        W = exp(-Diff_fun / (sigma_fun[j]*Sig))
