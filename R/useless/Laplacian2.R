Laplacian2 = function(P, tau, max_iter=100, eps=1e-9, verbose=FALSE){
  # same as Laplacian1 but with weights, not sigma
  dims = dim(P)
  funV = rep(0,max_iter)
  ...
  while(1){
    step = step + 1
    funV[step] = sum(w^2) +
      sum(apply(M, 3, function(x) norm(diag(m)-x-S, 'F'))*w)
    ...
    # update w
    Dmat = diag(n)
    dvec = apply(H, 3, function(x) norm(x-S, 'F'))
