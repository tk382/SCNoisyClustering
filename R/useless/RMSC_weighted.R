RMSC_weighted = function(P, tau, gamma, mu=1e-3, rho = 1.9, max_iter=100, eps=1e-9, verbose=FALSE){
  dims = dim(P);
  funV = rep(0,max_iter)
  ...
  while(1){
    ...
    funV[step] = sum(svd(S)$d) + norm(S, 'O')+sum(apply(E, 3, function(x) norm(x, 'F'))/(2*m*sigma)) + sum(sigma)/2
    ...
    #update sigma
    for (i in 1:n){
      norms = apply(E, 3, function(x) norm(x, 'F'))^2
      sigma = 2*sqrt(m)/(norms^2)
    }

    #update Ei and Wi
    for (i in 1:n){
      E[,,i] = mu/(1/(m*sigma[i]) + mu) * (P[,,i]-S-W[,,i]/mu)
  ...
  return(list(S=S, E=E, f = funV, sigma = sigma))
}
