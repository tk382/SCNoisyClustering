rankinfo = function(P, tau, gamma, r, mu=1e-3, eps = 1e-6, rho=1.9, max_iter=100, verbose=FALSE){
  m = p  = dim(P)[[1]]
  n      = dim(P)[[3]]
  S      = apply(P, c(1,2), sum)
  S_old  = matrix(0, m, p)
  ...
  DD     = DD * t(DD)
  Ltilde = (S-diag(m)) * DD
  J      = irlba(Ltilde, r, maxit=5000, r+20)$v
  w      = rep(1/n, n)
  step   = 0
  funV   = rep(0, max_iter)
  ...
  while(1){
    ...
    DD = DD * t(DD)
    Ltilde = (S-diag(m)) * DD
    J = irlba(Ltilde, r, maxit=5000, work=r+20)$v

    #update w
    Dmat = diag(n)
    dvec = apply(P, 3, function(x) norm(x-S, 'F'))^2 * w
    dvec = dvec/2
  ...
  pi = irlba(t(S), 1, maxit=5000, work=21)$v
